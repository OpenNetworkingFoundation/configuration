...
class XSDPlugin(plugin.PyangPlugin):
    def add_opts(self, optparser):
        optlist = [
            ...
            optparse.make_option("--xsd-no-any-augments",
                                 dest="xsd_no_any_augments",
                                 action="store_true",
                                 help="Do not generate xs:any elements that "
                                      "in principle allow augmentatsions")
            ]
...
def print_children(ctx, module, fd, children, indent, path,
    ...
    for c in children:
        uses = None
        if (ctx.opts.xsd_groups and hasattr(c, 'i_uses')):
            for s in c.i_uses:
                #print s.arg
                if s in c.parent.substmts:
                    uses = s
                    break
            if (uses != None and
                uses.i_grouping.parent.parent == None and
                uses.search_one('refine') is None):
                # do not inline this child; print a reference to the grouping
                # instead - only for top-level groupings
                if uses not in uses_list:
        ...
        if cn in ['container', 'list', 'leaf', 'leaf-list', 'anyxml',
            ...
            for child in c.i_children:
                ...
                print_children(ctx, module, fd,
                               child.i_children,
                               indent + '    ', path)
                if not ctx.opts.xsd_no_any_augments:
                    # allow for augments
...
def xsd_type_name(ctx, type, parent):
    if type.arg in yang_to_xsd_types:
        ...
        ref = None
        if ctx.opts.xsd_groups:
            path_type_spec = parent.i_leafref
            ref = find_leafref_ptr(ctx, parent, path_type_spec.path_spec,
                                   path_type_spec.path_)
        else:
            (ref, _pos) = parent.i_leafref_ptr
        if ref == None:
            raise error.EmitError("cannot find path %s for leafref"
                                  %  parent.i_leafref.path_)
...
def find_leafref_ptr(ctx, stmt, path_spec, path):
    ...
    def find_identifier(identifier):
        if util.is_prefixed(identifier):
            ...
            pmodule = statements.prefix_to_module(path.i_module, prefix,
                                                  stmt.pos, ctx.errors)
            if pmodule is None:
                raise NotFound
            return (pmodule, name)
    ...
    # we asume an absolute path here
    def follow_path(ptr, up, dn):
        path_list = []
        (pmodule, name) = find_identifier(dn[0])
        ptr = statements.search_child(pmodule.i_children, pmodule.i_modulename,
                                      name)
        if not statements.is_submodule_included(stmt, ptr):
            ptr = None
        if ptr is None:
            ...
            for inc in path.i_orig_module.search('include'):
                ...
                if submod is not None:
                    ptr = statements.search_child(submod.i_children,
                                                  submod.arg, name)
                    if ptr is not None:
                        break
            if ptr is None:
        ...
        dn = dn[1:]
        # this should never happen since the document should be valid
        ...
        while i < len(dn):
            if is_identifier(dn[i]) == True:
                ...
                while i < len(dn) and is_predicate(dn[i]) == True:
                    ...
                    pleaf = statements.search_child(ptr.i_key,
            ...
            if ptr.keyword in ['list', 'container', 'case', 'grouping',
                               ...
                               'notification']:
                ptr = statements.search_data_node(ptr.i_children,
                                                  module_name,
                                                  name)
                if ptr is None:
                    # this should not happen in a valid module
    ...
    try:
        ...
        if up != -1:
            raise error.EmitError("cannot use relative leafref statements"
                                  " in grouping when using --xsd-groups")
        if derefup > 0:
            ...
            if ptr.i_leafref is None:
                ...
                #        (ptr.arg, ptr.pos))
                raise NotFound
            # the referenced leaf should be expanded by validation already
